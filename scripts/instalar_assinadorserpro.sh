...
LIBGTK_MOD_DEB=$(basename $LIBGTK_MOD_URL)
LIBGTK0_DEB=$(basename $LIBGTK0_URL)
...
rm -rf "$WORK_DIR" "$SERPRO_DEB" "$LIBGTK_MOD_DEB" "$LIBGTK0_DEB" "$MODIFIED_SERPRO_DEB"
...
download_and_check $LIBGTK_MOD_URL $LIBGTK_MOD_SHA
download_and_check $LIBGTK0_URL $LIBGTK0_SHA
...
echo "--- ⬇️ Instalando os pacotes (Dependências + Modificado) ---"
# O APT instalará o arquivo Serpro modificado e as dependências:
apt install -y "./$LIBGTK0_DEB" "./$LIBGTK_MOD_DEB" "./$MODIFIED_SERPRO_DEB"
